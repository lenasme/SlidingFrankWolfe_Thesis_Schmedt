...
def compute_cheeger_set(grid_size, deltas, max_jumps, grid_size_coarse, cut_off, plot=True):
    ...
    if plot == True:
        ...
        plt.subplot(1,4,3)
        plt.imshow(operator_applied_on_ground_truth.imag, cmap= 'bwr')
        plt.title("Imaginärteil komplette Fourier")
        plt.subplot(1,4,4)
        plt.imshow(truncated_operator_applied_on_ground_truth.imag, cmap = 'bwr')
        plt.title("Imaginärteil abgeschnittene Fourier")
        plt.show()
