...
class SlidingOptimizerState:
    def __init__(self, initial_function, fourier, reg_param, y):
        self.function = None
        self.obs_tab = None
        self.diff_obs = None
        ...
        self.update_function(initial_function, fourier, reg_param, y)


    def update_function(self, new_function, fourier, reg_param, y):
        self.function = new_function
        self.obs_tab = self.function.compute_obs(fourier, version=1)
    ...
    def compute_gradient(self, fourier, reg_param):
        grad_weights = []
        grad_boundary_vertices = []
        grad_norm_squared = 0

        max_num_boundary_vertices = max(atom.support.num_boundary_vertices for atom in self.function.atoms)
        grad_area_weights = np.zeros((self.function.num_atoms, max_num_boundary_vertices, fourier.grid_size, 2))
        ...
        for i in range(self.function.num_atoms):
            ...
            grad_perimeter = support_i.compute_perimeter_rec_gradient()

            num_boundary_vertices = support_i.num_boundary_vertices
            grad_area = support_i.compute_weighted_area_rec_gradient(fourier, weights=grad_area_weights[i, :num_boundary_vertices])
...
class SlidingOptimizer:
    ...
    def perform_linesearch(self, fourier, y, reg_param, grad_weights, grad_boundary_vertices, grad_norm_squared, no_linesearch=False):
        t = self.step_size
        ...
        while not ag_condition:
            ...
            self.state.update_boundary_vertices(new_boundary_vertices, fourier, y, reg_param)
    ...
    def run(self, initial_function, fourier, reg_param, y, verbose=True):
        convergence = False
        obj_tab = []
        grad_norm_tab = []
        ...
        self.state = SlidingOptimizerState(initial_function, fourier, reg_param, y)

        while not convergence and iteration < self.max_iter:
            grad_weights, grad_boundary_vertices, grad_norm_squared = self.state.compute_gradient(fourier, reg_param)
            grad_norm_tab.append(np.sqrt(grad_norm_squared))
            obj_tab.append(self.state.obj)

            n_iter_linesearch, max_displacement = self.perform_linesearch(fourier, y, reg_param, grad_weights, grad_boundary_vertices, grad_norm_squared, no_linesearch=False)
            ...
            if self.num_iter_resampling is not None and iteration % self.num_iter_resampling == 0:
                ...
                self.state.update_function(new_function, fourier, reg_param, y)
