...
class SimpleFunction:
    ...
    def compute_obs(self, fourier, version=0):
        ...
        obs = np.zeros((self.num_atoms, max_num_triangles, fourier.grid_size**2), dtype = complex)
        #obs = np.zeros((self.num_atoms,  fourier.grid_size**2), dtype = complex)
        ...
        if version == 1:
            ...
            #res = np.zeros(fourier.grid_size**2, dtype = complex)
            #res = np.sum(obs, axis=0)
            #res = obs[0,:]
            #for i in range(self.num_atoms):
                #res += self.atoms[i].weight * np.sum(obs[i])
                #res += np.sum(obs[i], axis=0)
            res = np.sum(obs, axis = (0,1))
